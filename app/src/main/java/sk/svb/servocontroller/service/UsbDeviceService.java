package sk.svb.servocontroller.service;
...
import sk.svb.servocontroller.R;
...
public class UsbDeviceService extends Service {
    ...
    public int onStartCommand(Intent intent, int flags, int startId) {
        ...
        if (mUsbDevice == null){
            Log.d(TAG, "mUsbDevice == null");
        }else{
            Log.d(TAG, "mUsbDevice != null");
            Log.d(TAG, "mUsbDevice.getDeviceName: " + mUsbDevice.getDeviceName());
//            Log.d(TAG, "mUsbDevice.getSerialNumber: " + mUsbDevice.getSerialNumber());
            Log.d(TAG, "mUsbDevice.getDeviceProtocol: " + mUsbDevice.getDeviceProtocol());
        }
        if (!initDevice()) {
    ...
    private boolean initDevice() {
        ...
        if (mUsbConnection == null) {
            ...
            if (DEBUG) Log.d(TAG, "Opening USB success!");
        }

        Log.d(TAG, "getInterfaceCount: " + mUsbDevice.getInterfaceCount());
        for(int iface=0; iface < mUsbDevice.getInterfaceCount(); iface++) {
            ...
            Log.d(TAG, "usbInterface.getEndpointCount(): " + usbInterface.getEndpointCount());
            for (int i = 0; i < usbInterface.getEndpointCount(); i++) {

                Log.d(TAG, "usbInterface.getEndpoint(i).getType(): " + usbInterface.getEndpoint(i).getType());
                Log.d(TAG, "usbInterface.getEndpoint(i).getDirection(): " + usbInterface.getEndpoint(i).getDirection());
        ...
        public void onReceive(Context context, Intent intent) {
            ...
            if (SEND_DATA_INTENT.equals(action)) {
                ...
                if (dataToSend == null) {
                    ...
                    String text = "no_extra_in_intent " + DATA_EXTRA;
    ...
    private void startReceiverThread() {
        if (DEBUG) Log.d(TAG, "startReceiverThread");
...
//                    if (DEBUG) Log.d(TAG, "calling bulkTransfer() in");
                    ...
                    if (len > 0) {
                        if (DEBUG) Log.i(TAG, "data read length: " + len);
...
//                        if (DEBUG) Log.i(TAG, "zero data read!");
    ...
    private void startSenderThread() {
        if (DEBUG) Log.d(TAG, "startSenderThread");
    ...
    private class SenderThread extends Thread {
        ...
        public void run() {
            if (DEBUG) Log.d(TAG, "SenderThread running");
            Looper.prepare();
